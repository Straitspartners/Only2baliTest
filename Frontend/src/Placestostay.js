...
const Placestostay = () => {
  ...
  return (
    ...
    <div className="loading-bar" style={{ width: "35%" }}></div>
      ...
      <div className="grid">
      {categories.map((category) => {
        const index = selectedStays.indexOf(category.id);
        const preferenceLabel =
          index === 0 ? "Preference 1" : index === 1 ? "Preference 2" : "";

        return (
          <div
            key={category.id}
            className={`card ${index !== -1 ? "selected" : ""}`}
          >
            <img
              src={category.img}
              alt={category.name}
            ...
            <button
              type="button"
              className={`ps-card-button ${index !== -1 ? "selected" : ""}`}
              onClick={() => handleStaySelect(category.id)}
              disabled={
                selectedStays.length >= 2 && index === -1
              }
            >
              {index !== -1 ? `✓ ${preferenceLabel}` : `+ ${category.name}`}
            </button>
          </div>
        );
      })}
